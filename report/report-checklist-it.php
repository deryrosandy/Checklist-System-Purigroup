<?php
	...
	if (empty($_SESSION['username']) AND empty($_SESSION[password])) {
		...
		$userid = $_SESSION['id'];
		$branch_user = $db->users("id", $userid)->fetch();
		
		$branch = $branch_user->branch["name"];
		
		//$start_date = $_POST['start_date'];
		//$end_date = $_POST['end_date'];
		
		//list($bulana, $haria, $tahuna) = split ('/', $start_date);
		//list($bulanb, $harib, $tahunb) = split ('/', $end_date);
		
		//$startd = "$tahuna$bulana$haria";
		//$endd = "$tahunb$bulanb$harib";
		...
		$pdf->MultiCell(19.5,0.5,"Tanggal / Jam Check : " . $datenow . " / " . $timenow ,0,'L');    
		$outlet = $branch;
